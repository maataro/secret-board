  ...
  switch (req.method) {
    ...
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        const decoded = decodeURIComponent(body);  // content=投稿内容&oneTimeToken=トークン本体
        const dataArray = decoded.split('&');  // ['content="おはよう"', 'oneTimeToken="2we33fw2gs"']
...
// require('crypto').randomBytes(256).toString('hex');を実行して得られた値
// 16 進数で表されたランダムなバイト列を文字列として得られる
const secretKey =
...
/**
 * originalIDとユーザー名を受け取って、
 * originalID + ユーザー名　+ シークレットキー　からハッシュ値を作成し、
 * originalID_ハッシュ値　をトラッキングIDとして返す。
